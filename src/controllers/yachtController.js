...
export const editYacht = async (req, res, next) => {
  try {
    ...
    if (req.files && req.files.primaryImage && req.files.primaryImage[0]) {
      try {
        const file = req.files.primaryImage[0];
        yachtData.primaryImage = await uploadToCloudinary(file.path, 'phuket-sailing/yachts/primaryImage');
      } catch (uploadError) {
    ...
    if (galleryImageFiles.length > 0) {
      ...
      for (const file of galleryImageFiles) {
        try {
          const url = await uploadToCloudinary(file.path, 'phuket-sailing/yachts/galleryImages');
